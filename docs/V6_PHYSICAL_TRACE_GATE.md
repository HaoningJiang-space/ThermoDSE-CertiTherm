...
| Critical | Communication heat cannot be located from `link_hops` because its coordinate index aliases rows and external columns. | Independent event capture and route reconstruction with parity against aggregate NoC/NoP energy on all registered candidates. | **OPEN** |
