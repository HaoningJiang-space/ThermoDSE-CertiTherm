...
# (tiles_x, tiles_y, cut_x, cut_y) -- area and how it is cut into dies.
...
def main() -> None:
    ...
    results = []
    for index, (tx, ty, cx, cy) in enumerate(GRID):
        arch = {"architecture_id": "sweep_%02d" % index, "chiplet_x": str(tx),
                "chiplet_y": str(ty), "cut_x": str(cx), "cut_y": str(cy), **BASE}
        ...
        try:
            ...
            print("%-10s %d x %d cut %d x %d  SKIPPED  %s: %s" % (
                arch["architecture_id"], tx, ty, cx, cy, type(exc).__name__, str(exc)[:60]),
                flush=True)
            continue
        for wl in workloads:
            ...
            results.append({"arch": arch["architecture_id"], "tiles": [tx, ty], "cut": [cx, cy],
                            ...
                            "yield": y, "edyp": edyp})
            print("%-10s %dx%d cut %dx%d dies %d  %-12s peak %7.2f  tau* %7.1f%%  beta* %6.2f%%"
                  "  Y %.4f  EDYP %8.3f  (%.0fs)" % (
                      arch["architecture_id"], tx, ty, cx, cy, cx * cy, wl, peak,
