...
def validate_gate(m: dict, view: dict) -> dict:
    ...
    _require(stored == reg,
             ...
             f"{REGISTRATION.relative_to(ROOT)}; one of them has drifted")
...
def render(m: dict, view: dict, g: dict, ex: dict, manifest_path: Path) -> str:
    ...
    A(f"Recomputed from the manifest rows by `research/triangle/v61_render_evidence.py`: the "
      ...
      f"`{REGISTRATION.relative_to(ROOT)}`, not the manifest's own copy of them). Any "
    ...
    A("## Gate")
    A("")
    A(f"Recomputed against `{REGISTRATION.relative_to(ROOT)}`, whose registered tuple the "
    ...
    try:
        shown = manifest_path.resolve().relative_to(ROOT)
    except ValueError:
        shown = manifest_path
    A(f"Manifest: `{shown}`")
    return "\n".join(L) + "\n"
