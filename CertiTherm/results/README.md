...
| `G2_GATE_REPORT.md` | superseded unsound pilot | max-min bug record only |
| `G2_SOUNDNESS_REPORT.md`, `decisive_experiment.json` | intermediate synthetic pilot | debugging the zero-lower-bound LP path only |
| `G3_REPORT.md`, `g3_measurement_selection.json` | legacy invalidated | generated by the duplicated max-min measurement path before G2 passed |
| `G3_FULL_REPORT.md`, `g3_full_empirical.json` | legacy invalidated | mislabeled aggregate workload data, package-aliased operator, nested-bound sensitivity only |
| `G4_REPORT.md`, `g4_acquisition.json` | implementation repair / legacy invalidated | no numerical G4 claim; the old result is a single-candidate early return on truncated/padded input |
| `G2_CORRECTION_REPORT.md` | corrected physical G2 evidence | narrow G2 certificate/witness claim with stated limitations |

The corrected G2 run keeps its large replay artifacts outside Git. The
committed physical-input registry and `G2_CORRECTION_REPORT.md` provide the
content hashes, execution commit, scientific digest, and claim boundary.
